"""Gold analysis and balanced BUY/SELL signal generation for SHIVAY AI."""
...
LOGGER = logging.getLogger("shivay.gold")
IST = ZoneInfo("Asia/Kolkata")
GOLD_SYMBOL = str(getattr(config, "GOLD_SYMBOL", "GOLD FUT")).strip() or "GOLD FUT"
YAHOO_FALLBACK_SYMBOL = str(getattr(config, "GOLD_YAHOO_SYMBOL", "GC=F")).strip() or "GC=F"
CACHE_SECONDS = max(180, int(getattr(config, "GOLD_CACHE_SECONDS", 300)))
STALE_MINUTES = max(20, int(getattr(config, "GOLD_STALE_MINUTES", 90)))
DATA_RETRIES = max(1, min(int(getattr(config, "GOLD_DATA_RETRIES", 2)), 3))
RETRY_DELAY = max(0.0, min(float(getattr(config, "GOLD_RETRY_DELAY", 0.5)), 2.0))
MCX_OPEN = clock_time(
...
def _project_gold_data() -> tuple[Any, str] | tuple[None, None]:
    """Use a project-native Gold mapping when a current/future feed provides one."""
    try:
        manager = importlib.import_module("provider_manager").get_provider_manager()
        value = manager.get_verified_market_data("GOLD FUT", period="15d", interval="5m")
        verified = (
        ...
        return (value, "VERIFIED_MCX:GOLD") if verified else (None, None)
    except Exception:
        return None, None


def _download_yahoo_gold() -> tuple[Any, str] | tuple[None, None]:
    ...
    for attempt in range(1, DATA_RETRIES + 1):
        try:
            ...
            LOGGER.warning("Temporary Gold data failure (%s/%s)", attempt, DATA_RETRIES)
...
def _load_records() -> tuple[list[dict[str, Any]], str | None]:
    native, source = _project_gold_data()
    if native is not None:
        records = _records_from_project(native)
        if records:
            return records, source
    frame, source = _download_yahoo_gold()
    if frame is not None:
        return _records_from_frame(frame), source
    return [], None


def _silver_confirmation() -> dict[str, Any]:
    try:
        if importlib.util.find_spec("silver") is None:
            return {"available": False, "direction": "UNAVAILABLE"}
        module = importlib.import_module("silver")
    except Exception:
        return {"available": False, "direction": "UNAVAILABLE"}
    for name in ("get_silver_trend", "get_silver_signal", "analyze_silver"):
        ...
        try:
            ...
            if isinstance(value, Mapping):
                raw = value.get("trend", value.get("signal", value.get("direction")))
            else:
                raw = value
            direction = _direction(raw)
            if direction != "UNAVAILABLE":
                return {"available": True, "direction": direction, "source": "silver"}
        except Exception:
            LOGGER.warning("Silver confirmation unavailable")
    return {"available": False, "direction": "UNAVAILABLE"}
...
def _empty_analysis(reason: str, busy: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        "reasons": [reason],
        "inputs": {
            "silver": {"available": False, "direction": "UNAVAILABLE"},
            "usd": {"available": False},
...
def _trade_plan(price: float, atr_value: float, side: str) -> dict[str, float]:
    try:
        ...
        LOGGER.warning("Project risk engine unavailable for Gold")
...
def _build_analysis() -> dict[str, Any]:
    ...
    if len(records) < 240:
        return _empty_analysis("Gold data is unavailable or has insufficient history")
    records = records[-350:]
    ...
    silver = _silver_confirmation()
    try:
        global_context = importlib.import_module("global_metals_context").get_global_metals_context()
    except Exception:
        global_context = {"gold_bias": "SIDEWAYS", "gold_confidence": 0, "degraded": True}
    ...
    if silver.get("available"):
        if silver["direction"] == "BULLISH":
            buy_score += 3
        elif silver["direction"] == "BEARISH":
            sell_score += 3
    if global_context.get("gold_bias") == "BULLISH":
        buy_score += min(10, _number(global_context.get("gold_confidence")) / 10)
    elif global_context.get("gold_bias") == "BEARISH":
        sell_score += min(10, _number(global_context.get("gold_confidence")) / 10)
    ...
    # Global-macro directional veto: a gold trade must not run directly against
    # a fresh, confident global backdrop (XAUUSD/COMEX/DXY/USDINR). Confidence
    # only reaches the threshold when the primary feed is fresh, so a stale or
    # degraded macro read never blocks a signal.
    macro_bias = str(global_context.get("gold_bias") or "SIDEWAYS").upper()
    macro_confidence = _number(global_context.get("gold_confidence"))
    macro_opposes = (
    ...
    if macro_opposes:
        reasons.append(f"Global gold macro is {macro_bias.title()} ({int(macro_confidence)}%) against a {leading_side.title()}")
    if stale:
        reasons.append("Gold quote is stale")
    ...
    return {
        ...
        "global_proxy": None if indian_mcx_verified else "COMEX Gold",
        "data_available": True,
...
def analyze_gold(force_refresh: bool = False) -> dict[str, Any]:
    """Generate a cached Gold analysis without blocking concurrent scans."""
    ...
    with _CACHE_LOCK:
        ...
        quote = None
        try:
            quote = importlib.import_module("mcx_temporary_provider").MCXTemporaryProvider().get_quote("MCX GOLD")
        except Exception:
            pass
        ...
        try:
            ...
            LOGGER.exception("Gold analysis degraded safely")
            result = _empty_analysis("Gold analysis failed because one or more inputs are unavailable")
            ...
            try:
                if quote and _number(quote.get("price")) > 0:
                    ...
                    result.update(data_available=True, indian_mcx_quote=True, price=_number(quote["price"]),
                                  ...
                                  stale=False, trend=context.get("gold_bias", "SIDEWAYS"), regime=context.get("gold_bias", "SIDEWAYS"), degraded=True,
                                  global_confirmation=context,
                                  reasons=["Live MCX quote received; exchange timestamp and candles are unavailable"])
            except Exception:
                LOGGER.warning("Gold quote-only fallback unavailable")
...
def scan_gold(force_refresh: bool = False) -> dict[str, Any]:
    """Run one non-overlapping Gold scan."""
    if not _SCAN_LOCK.acquire(blocking=False):
        return _empty_analysis("Another Gold scan is already running", busy=True)
    try:
        return analyze_gold(force_refresh)
    finally:
        _SCAN_LOCK.release()


def get_gold_signal() -> str:
    return str(analyze_gold().get("signal", "NO TRADE"))


def get_gold_trend() -> str:
    return str(analyze_gold().get("trend", "UNKNOWN"))


def get_gold_regime() -> str:
    return str(analyze_gold().get("regime", "UNKNOWN"))


def get_gold_confidence() -> int:
    return int(_number(analyze_gold().get("confidence")))


def get_gold_trade_plan() -> dict[str, Any]:
    result = analyze_gold()
    return {
...
def is_gold_tradeable() -> bool:
    return bool(analyze_gold().get("tradeable", False))


def format_gold_details(analysis: Mapping[str, Any] | None = None) -> str:
    data = dict(analysis or analyze_gold())
    ...
    return (
        ...
        f"Entry Zone: {data.get('entry_zone', (0, 0))[0]} — {data.get('entry_zone', (0, 0))[1]}\n"
        ...
        f"REASONS\n{reason_text}\n\n"
        "Gold signals are probability-based, not guaranteed. Unverified values never authorize a trade."
    )


def format_gold_report(analysis: Mapping[str, Any] | None = None) -> str:
    from telegram_service import format_commodity_signal
    return format_commodity_signal(dict(analysis or analyze_gold()), "GOLD")
...
async def send_gold_report(
    ...
    if application is None or not hasattr(application, "bot"):
        return False
    data = dict(analysis) if analysis is not None else await asyncio.to_thread(scan_gold, force_refresh)
    today = _now().date()
    with _CACHE_LOCK:
    ...
    try:
        ...
        LOGGER.exception("Gold-report recipients are unavailable")
        with _CACHE_LOCK:
            _sent_state[today]["in_flight"] = False
        return False
    text = format_gold_report(data)
    delivered = False
    for user in users:
        try:
            ...
            LOGGER.warning("Gold report delivery failed")
    if delivered and data.get("tradeable") and _monitor_supported(str(data.get("symbol"))):
        try:
            ...
            LOGGER.warning("Gold trade could not be registered with trade monitor")
    with _CACHE_LOCK:
        if delivered:
